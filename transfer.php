...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($senderID) || empty($recipientID) || empty($amount)) {
        ...
        $senderSQL = "SELECT name, balance FROM randompeople WHERE id = $senderID";
        $senderResult = $conn->query($senderSQL);
        $senderRow = $senderResult->fetch_assoc();

        if ($senderRow) {
            // Retrieve the recipient's details
            $recipientSQL = "SELECT name, balance FROM randompeople WHERE id = $recipientID";
            ...
            if ($recipientRow) {
                ...
                if ($amount <= $senderRow["balance"]) {
                    ...
                    $updateSenderSQL = "UPDATE randompeople SET balance = $newSenderBalance WHERE id = $senderID";
                    $conn->query($updateSenderSQL);

                    // Update the recipient's balance in the database
                    $updateRecipientSQL = "UPDATE randompeople SET balance = $newRecipientBalance WHERE id = $recipientID";
...
$customersSQL = "SELECT id, name FROM randompeople WHERE id != $senderID";
$customersResult = $conn->query($customersSQL);

// Retrieve the sender's details
$senderSQL = "SELECT id, name, balance FROM randompeople WHERE id = $senderID";
$senderResult = $conn->query($senderSQL);
$senderRow = $senderResult->fetch_assoc();
